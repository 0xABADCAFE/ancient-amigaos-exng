 ...
 *  Revision 0.3
 *
 *  Fixed 3 logical errors since version 0.0
 *
 *  1) fixed bad Duff calc in 64-bit CopyMemQuick() case
 *  2) fixed error in overall length reduction in pre Duff alignments
 *  3) fixed error in pre Duff alignment unrolling
 *
 *  Added better commenting, especially concerning Duff's Device calcs.
 *  For my own sanity, more than anything else. This was easier in asm!
 *
 *  Added some simple sanity checks to the function parameters.
 *
 *  If I find any more bugs after I send this I'll go mental :-D
 *
 */
...
extern "C" void Karl_CopyMemQuick(CONST APTR source, APTR dest, ULONG size);
extern "C" void Karl_CopyMem(CONST APTR source, APTR dest, ULONG size);
